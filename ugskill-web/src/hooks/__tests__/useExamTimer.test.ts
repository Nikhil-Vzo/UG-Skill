...
describe('useExamTimer', () => {
  ...
  it('connects to /exam and joins attempt on mount', () => {
    renderHook(() => useExamTimer('attempt-123', vi.fn()));
  ...
  it('updates time left on timer:init and timer:tick', () => {
    const { result } = renderHook(() => useExamTimer('attempt-123', vi.fn()));
    ...
    act(() => {
      initCallback({ remainingSecs: 3600 });
    });
    expect(result.current.timeLeft).toBe(3600);

    act(() => {
      tickCallback({ remainingSecs: 3599 });
    });
