...
export interface IProctoringEvent extends Document {
  ...
  examId: string;
  studentId: string;
  type: 'gaze_away' | 'talking' | 'no_face' | 'multiple_faces' | 'phone_detected' | 'eyes_closed' | 'tab_switch' | 'copy_paste' | 'fullscreen_exit' | 'admin_flag' | 'admin_terminate' | 'exam_start' | 'exam_submit' | 'heartbeat';
...
const ProctoringEventSchema = new Schema({
  ...
  type: { 
    type: String, 
    required: true,
    enum: ['gaze_away', 'talking', 'no_face', 'multiple_faces', 'phone_detected', 'eyes_closed', 'tab_switch', 'copy_paste', 'fullscreen_exit', 'admin_flag', 'admin_terminate', 'exam_start', 'exam_submit', 'heartbeat']
  },
  severity: { 
    type: String, 
    required: true,
